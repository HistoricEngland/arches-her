        ...
        try:
            with transaction.atomic():
                with connection.cursor() as cursor:
                    ...
                    #         ( SELECT jsonb_agg(bscr.statement_of_responsibility) AS jsonb_agg
                    #             FROM bibliographic_source.bibliographic_source_creation bscr
                    #             WHERE (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = bscr.resourceinstanceid) AS statement_of_authority,
                    ...
                    #             CASE
                    #                 WHEN (ecr.url ->> 'url'::text) LIKE '%doi.org%'::text THEN ecr.url ->> 'url'::text
                    #                 ELSE NULL::text
                    #             END AS source_digital_object_identifier,
                    #             CASE
                    #                 WHEN (ecr.url ->> 'url'::text) NOT LIKE '%doi.org%'::text THEN ecr.url ->> 'url'::text
                    #                 ELSE NULL::text
                    #             END AS source_url
                    #     FROM monument.bibliographic_source_citation bsc
                    #         LEFT JOIN bibliographic_source.bibliographic_source_names bsn ON (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = bsn.resourceinstanceid
                    #         LEFT JOIN bibliographic_source.external_cross_references ecr ON (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = ecr.resourceinstanceid
                    #         LEFT JOIN bibliographic_source.publication bsp ON (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = bsp.resourceinstanceid
                    #     UNION ALL
                    #     SELECT bsc.resourceinstanceid,
                    #         bsn.bibliographic_source_name AS information_source_title,
                    #         ( SELECT jsonb_agg(bscr.statement_of_responsibility) AS jsonb_agg
                    ...
                    #                 ELSE NULL::text
                    #             END AS source_url
                    #     FROM historic_aircraft.bibliographic_source_citation bsc
                    #         LEFT JOIN bibliographic_source.bibliographic_source_names bsn ON (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = bsn.resourceinstanceid
                    #         LEFT JOIN bibliographic_source.external_cross_references ecr ON (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = ecr.resourceinstanceid
                    #         LEFT JOIN bibliographic_source.publication bsp ON (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = bsp.resourceinstanceid
                    #     UNION ALL
                    ...
                    #                 ELSE NULL::text
                    #             END AS source_url
                    #     FROM maritime_vessel.bibliographic_source_citation bsc
                    #         LEFT JOIN bibliographic_source.bibliographic_source_names bsn ON (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = bsn.resourceinstanceid
                    #         LEFT JOIN bibliographic_source.external_cross_references ecr ON (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = ecr.resourceinstanceid
                    #         LEFT JOIN bibliographic_source.publication bsp ON (((bsc.bibliographic_source_citation -> 0) ->> 'resourceId'::text)::uuid) = bsp.resourceinstanceid
                    #     WITH NO DATA;
                    # """)
                    # cursor.execute("""
                    #     ALTER TABLE hapi.monument_sources_mv
                    #         OWNER TO postgres;
                    # """)
                    # cursor.execute("""
                    #     CREATE INDEX monument_sources_resourceinstanceid
                    #         ON hapi.monument_sources_mv USING btree
                    #         (resourceinstanceid)
                    #         TABLESPACE pg_default;
                    # """)
                    cursor.execute("""
                    ...
                    #             ), artefact_ids AS (
                    #             SELECT
                    #                 (associated_resources.artefact ->> 'resourceId'::text)::uuid AS resourceinstanceid,
                    #                 associated_resources.resourceinstanceid AS main_resourceinstanceid
                    #             FROM associated_resources
                    #             ), vessel_data AS (
                    #             SELECT
                    #                 mvcp.resourceinstanceid,
                    ...
                    #                 mvcp.cultural_period,
                    #                 mvcp.main_construction_material,
                    #                 mvcp.covering_material,
                    #                 ai.main_resourceinstanceid
                    #             FROM maritime_vessel.construction_phases mvcp
                    #                 JOIN artefact_ids ai ON mvcp.resourceinstanceid = ai.resourceinstanceid
                    #             ), vessel_types AS (
                    #             SELECT
                    #                 vd_1.resourceinstanceid,
                    #                 jsonb_agg(DISTINCT pv.value) AS vessel_types
                    #             FROM vessel_data vd_1
                    #                 JOIN "values" pv ON pv.valueid = ANY (vd_1.maritime_vessel_type)
                    #             GROUP BY vd_1.resourceinstanceid
                    #             ), cultural_periods AS (
                    #             SELECT
                    #                 vd_1.resourceinstanceid,
                    #                 jsonb_agg(DISTINCT pn.period_name) AS cultural_periods
                    #             FROM vessel_data vd_1
                    #                 JOIN LATERAL jsonb_array_elements(vd_1.cultural_period) cp_1(value) ON true
                    #                 JOIN period.period_names pn ON pn.resourceinstanceid = ((cp_1.value ->> 'resourceId'::text)::uuid)
                    #             GROUP BY vd_1.resourceinstanceid
                    #             ), materials AS (
                    #             SELECT
                    #                 vd_1.resourceinstanceid,
                    #                 jsonb_agg(DISTINCT pv.value) AS materials
                    #             FROM ( SELECT vessel_data.resourceinstanceid,
                    #                         unnest(vessel_data.main_construction_material) AS material
                    #                     FROM vessel_data
                    #                     UNION ALL
                    #                     SELECT vessel_data.resourceinstanceid,
                    #                         unnest(vessel_data.covering_material) AS material
                    #                     FROM vessel_data) vd_1
                    ...
                    #         m.materials
                    #     FROM vessel_data vd
                    #         LEFT JOIN vessel_types vt ON vd.resourceinstanceid = vt.resourceinstanceid
                    #         LEFT JOIN cultural_periods cp ON vd.resourceinstanceid = cp.resourceinstanceid
                    #         LEFT JOIN materials m ON vd.resourceinstanceid = m.resourceinstanceid
                    #     WITH NO DATA;
                    # """)
                    cursor.execute("""
                    ...
                    #         cp.cultural_periods,
                    #         m.materials
                    #     FROM artefact_data ad
                    #     JOIN artefact_types at ON ad.resourceinstanceid = at.resourceinstanceid
                    #     JOIN cultural_periods cp ON ad.resourceinstanceid = cp.resourceinstanceid
                    #     JOIN materials m ON ad.resourceinstanceid = m.resourceinstanceid
                    #     JOIN artefact_ids ai ON ad.resourceinstanceid = ai.associated_resourceinstanceid
                    #     WITH NO DATA;
                    # """)
                    cursor.execute("""
                        CREATE MATERIALIZED VIEW hapi.object_finds
